...
> [!IMPORTANT]
> Antes de iniciar, copie los archivos de ejemplo y renombrelos a `.env`:
>
...
- **Carpeta Raíz de la Feature:** Debe usar el prefijo **`Manage`** seguido del nombre de la página en _PascalCase_ (ejemplo: `ManageTodayPage`, `ManageUserDashboard`).
- **Componentes:** Se nombran en _PascalCase_ (ejemplo: `TableComponent.tsx`).
- **Hooks:** Deben empezar siempre con el prefijo **`use`** en _camelCase_ (ejemplo: `useFetchTodayData.ts`).
- **Services / Utils / Types:** Se nombran en _camelCase_ describiendo su función específica (ejemplo: `formatDate.ts`, `authService.ts`).
